...
REMOTE_DATA_ROOT="/root/autodl-tmp/data/libero_medium"
REMOTE_POLICY="/root/autodl-tmp/checkpoints/openvla-oft-libero10-traj1-rl"
REMOTE_SPATIAL="/root/autodl-tmp/checkpoints/facebook-dinov2-base"
...
for SEED in 7 11 19; do
  ...
  echo "[spare-lite-sweep] running seed=${SEED}"
  PYTHONPATH="${REMOTE_ROOT}" \
  PYTHONUNBUFFERED=1 \
  /root/miniconda3/bin/python -m spare_lite.rl_spare_latent_smoke \
    --jsonl-path "${REMOTE_DATA_ROOT}/medium_train.jsonl" \
...
echo "[spare-lite-sweep] finished. outputs in ${REMOTE_OUT_DIR}"
